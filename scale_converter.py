...
class ScaleConverter:
    ...
    def __init__(self):
        ...
        self._key = {
            self._notes[F][FL].get_note(): {'orientation': DF, 'offset': -8},
            self._notes[C][FL].get_note(): {'orientation': DF, 'offset': -7},
            self._notes[G][FL].get_note(): {'orientation': DF, 'offset': -6},
            self._notes[D][FL].get_note(): {'orientation': FL, 'offset': -5},
            self._notes[A][FL].get_note(): {'orientation': FL, 'offset': -4},
            self._notes[E][FL].get_note(): {'orientation': FL, 'offset': -3},
            self._notes[B][FL].get_note(): {'orientation': FL, 'offset': -2},
            self._notes[F][NT].get_note(): {'orientation': FL, 'offset': -1},
            self._notes[C][NT].get_note(): {'orientation': C,  'offset': 0},
            self._notes[G][NT].get_note(): {'orientation': SH, 'offset': 1},
            self._notes[D][NT].get_note(): {'orientation': SH, 'offset': 2},
            self._notes[A][NT].get_note(): {'orientation': SH, 'offset': 3},
            self._notes[E][NT].get_note(): {'orientation': SH, 'offset': 4},
            self._notes[B][NT].get_note(): {'orientation': SH, 'offset': 5},
            self._notes[F][SH].get_note(): {'orientation': SH, 'offset': 6},
            self._notes[C][SH].get_note(): {'orientation': DS, 'offset': 7},
            self._notes[G][SH].get_note(): {'orientation': DS, 'offset': 8},
            self._notes[D][SH].get_note(): {'orientation': DS, 'offset': 9},
            self._notes[A][SH].get_note(): {'orientation': DS, 'offset': 10},
            self._notes[E][SH].get_note(): {'orientation': DS, 'offset': 11},
            self._notes[B][SH].get_note(): {'orientation': DS, 'offset': 12},
        }
    ...
    def convert_scale(self, scale, root, acc=NT):
        """ Returns a list containing the valid note names of a scale given its key center. """
        ...
        for i, num in enumerate(scale.get_scale_code()):
            if num == NOTE_ON:
                converted_scale.append(scale_order[i].get_note())
    ...
    def get_note_order_by_key(self, root, acc=NT):
        ...
        try:
            order_map = self._order_map[self._key[root + acc]['orientation']]
        except KeyError:
            print(KEY_ERROR)
            return None
